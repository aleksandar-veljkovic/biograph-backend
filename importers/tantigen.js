            ...
            for (const filename of files) {
                ...
                for (const row of table) {
                    ...
                    if (key == 'Antigen Name') {
                        ...
                        geneEntityId = await bg.createEntityNode('Gene', antigenName);
                        await bg.createIdentifierNode(geneEntityId, 'name', 'Gene Name', antigenName);
                        
                        await bg.createEntityEdge(geneEntityId, antigenEntityId, 'IS_ANTIGEN', {});
                    }
                    ...
                    if (key == 'UniProt ID') {
                        ...
                        await bg.createEntityEdge(proteinEntityId, antigenEntityId, 'IS_ANTIGEN', {});

                        if (geneEntityId != null) {
                            await bg.createEntityEdge(proteinEntityId, geneEntityId, 'FROM_GENE', {});    
                        }
                    }
                    ...
                    if (key == 'Isoforms') {
                        ...
                        for (const isoform of isoforms) {
                            ...
                            if (antigenEntityId != isoNodeId) {
                                await bg.createEntityEdge(antigenEntityId, isoNodeId, 'IS_ISOFORM', {});
                                await bg.createEntityEdge(isoNodeId, antigenEntityId, 'IS_ISOFORM', {});
                            }
                        }
                    }
                    ...
                    if (key == 'Mutation entries') {
                        ...
                        for (const entry of mutationEntries) {
                            ...
                            if (antigenEntityId != mutationNodeId) {
                                await bg.createEntityEdge(antigenEntityId, mutationNodeId, 'IS_MUTATION_ENTRY', {});
                                await bg.createEntityEdge(mutationNodeId, antigenEntityId, 'IS_MUTATION_ENTRY', {});
                            }
                        }
                    }
                    ...
                    if (key == 'T cell epitope') {
                        ...
                        if (row[1].trim() != "Epitope sequence") {
                            ...
                            await bg.createEntityEdge(antigenEntityId, epitopeEntityId, 'HAS_EPITOPE', {});
                        }
                    }
                    ...
                    if (key == 'HLA ligand') {
                        ...
                        if (row[1] != 'Ligand Sequence' && row[1] != 'Predicted HLA binders' && row[1] != 'Antigen sequence') {
                            ...
                            await bg.createEntityEdge(antigenEntityId, ligandEntityId, 'HAS_LIGAND', {});
                        }
                    }
                }
            }
